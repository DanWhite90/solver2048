...
fn process_line(line: &DecodedLine) -> LineStackingResult {
  let mut new_line: DecodedLine = [0; GRID_SIDE];
...
fn traverse_row(row: &DecodedLine, position: usize, moves_table: &mut HashMap<u32, LineStackingResult>) {
...
mod tests {
  ...
  pub fn test_grid_get_decoded_state() {
    ...
    let grid: GameGrid = GameGrid::new(&decoded_grid);
